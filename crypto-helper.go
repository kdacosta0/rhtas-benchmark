...
func init() {
	// Validate required environment variables at startup — fail fast before accepting traffic.
	tsaURL = os.Getenv("TSA_URL")
	if tsaURL == "" {
		log.Fatal("[FATAL] TSA_URL environment variable must be set")
	}
	...
	if strings.EqualFold(os.Getenv("INSECURE_SKIP_TLS_VERIFY"), "true") {
		log.Println("[WARN] TLS certificate verification is disabled via INSECURE_SKIP_TLS_VERIFY")
...
func timestampHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Cap request body at 1 MB to prevent OOM from oversized payloads.
	const maxBodySize = 1 << 20 // 1 MB
	r.Body = http.MaxBytesReader(w, r.Body, maxBodySize)
	signatureToTimestamp, err := io.ReadAll(r.Body)
	if err != nil {
		http.Error(w, fmt.Sprintf("[ERROR] Error reading request body (body may exceed 1MB limit): %v", err), http.StatusBadRequest)
		return
	}
	...
	tsaResp, err := httpClient.Post(tsaURL, "application/json", bytes.NewBuffer(tsaRequestBytes))
	if err != nil {
	...
	if _, err := w.Write(tsaTimestampBytes); err != nil {
		log.Printf("[WARN] Failed to write timestamp response: %v", err)
	}
}
